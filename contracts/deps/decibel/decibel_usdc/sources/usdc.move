module decibel_dex::usdc {
    ...
    public fun metadata(): object::Object<fungible_asset::Metadata>
        acquires USDCRef
    {
        *&borrow_global<USDCRef>(@decibel_dex).metadata
    }
    public entry fun burn(p0: address, p1: u64)
        acquires USDCRef
    {
        let _v0 = borrow_global<USDCRef>(@decibel_dex);
    ...
    public fun transfer(p0: &signer, p1: address, p2: u64)
        acquires USDCRef
    {
        let _v0 = *&borrow_global<USDCRef>(@decibel_dex).metadata;
        ...
        acquires AdminConfig, USDCRef
    {
        ...
        let _v2 = borrow_global<AdminConfig>(@decibel_dex);
        if (smart_table::contains<address,bool>(&_v2.admins, _v1)) _v0 = true else _v0 = *&_v2.allow_public_minting;
        assert!(_v0, 2);
        primary_fungible_store::mint(&borrow_global<USDCRef>(@decibel_dex).mint_ref, p1, p2);
    }
        ...
        acquires AdminConfig
    {
        ...
        let _v1 = borrow_global_mut<AdminConfig>(@decibel_dex);
    ...
    {
        let _v0;
        let _v1 = borrow_global<RestrictedMint>(@decibel_dex);
    ...
    public fun can_mint(p0: address): bool
        acquires AdminConfig
    {
        let _v0 = borrow_global<AdminConfig>(@decibel_dex);
        ...
        acquires AdminConfig, RestrictedMint
    {
        ...
        let _v2 = borrow_global_mut<RestrictedMint>(@decibel_dex);
    ...
    public fun get_admin_count(): u64
        acquires AdminConfig
    {
        *&borrow_global<AdminConfig>(@decibel_dex).admin_count
    }
    public fun is_admin(p0: address): bool
        acquires AdminConfig
    {
        smart_table::contains<address,bool>(&borrow_global<AdminConfig>(@decibel_dex).admins, p0)
    }
    public fun is_public_minting_allowed(): bool
        acquires AdminConfig
    {
        *&borrow_global<AdminConfig>(@decibel_dex).allow_public_minting
    }
    ...
    {
        let _v0 = signer::address_of(p0);
        let _v1 = borrow_global_mut<RestrictedMint>(@decibel_dex);
        ...
        primary_fungible_store::mint(&borrow_global<USDCRef>(@decibel_dex).mint_ref, _v0, p1);
    }
    public fun restricted_mint_daily_reset_timestamp(): u64
        acquires RestrictedMint
    {
        *&(&borrow_global<RestrictedMint>(@decibel_dex).daily_restricted_mint).trigger_reset_mint_ts
    }
    public fun restricted_mint_daily_reset_timestamp_for(p0: address): u64
        acquires RestrictedMint
    {
        let _v0 = &borrow_global<RestrictedMint>(@decibel_dex).total_restricted_mint_per_owner;
        let _v1 = 0;
        let _v2 = &_v1;
